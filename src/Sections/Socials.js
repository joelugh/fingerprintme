...
function Socials() {
  ...
  React.useEffect(() => {
    ...
    /* from: http://webkay.robinlinus.com/scripts/social-media.js */
    networks.forEach(function(network) {
        try {
            var img = document.createElement('img');
            img.src = network.url;
            img.onload = function() {
                console.log(network.name + ' authenticated')
                setRows(rows => [
                    ...rows,
                    {name: network.name, value:'Authenticated'},
                ]);
            };
            img.onerror = function() {
                console.log(network.name + ' not authenticated')
            };
        } catch(err) {
            console.log("error checking " + network.name);
        }
    });
  ...
  return <TypeGroup>
        <Type
            command={'./probe social network accounts'}
        />
        {rows.length === 0 &&
            <Type
                result={`Not signed in to any socials.`}
            />
        }
